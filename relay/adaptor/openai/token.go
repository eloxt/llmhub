...
func InitTokenEncoders() {
	...
	models, err := model.CacheGetModelList()
	if err != nil {
		logger.FatalLog(fmt.Sprintf("failed to get model list: %s", err.Error()))
	}
	for _, modelStr := range models {
		if strings.HasPrefix(modelStr, "gpt-3.5") {
			tokenEncoderMap[modelStr] = gpt35TokenEncoder
		} else if strings.HasPrefix(modelStr, "gpt-4o") {
			tokenEncoderMap[modelStr] = gpt4oTokenEncoder
		} else if strings.HasPrefix(modelStr, "gpt-4") {
			tokenEncoderMap[modelStr] = gpt4TokenEncoder
		} else {
			tokenEncoderMap[modelStr] = nil
		}
	}
